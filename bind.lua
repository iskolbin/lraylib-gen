...
return function( conf, defs, custom )
	...
	for structName, struct in pairs( defs.structs ) do
		...
		if primitive and not struct.complex then
			isPrimitiveStruct[structName] = struct.fields
		end
	end
	local prefix = (conf.prefix or '') .. '_' .. conf.libname .. '_'
	...
	for structName, struct in pairs( defs.structs ) do
		if not struct.pass then
			...
			local function plurify( name )
				if name:match( 'rices$' ) then
					return name:sub( 1, -6 ) .. 'rixAt'
				elseif name:match( 'ices$' ) then
					return name:sub( 1, -5 ) .. 'exAt'
